...
class Day17(val lines: List<String>) : AocChallenge(2024, 17) {
    ...
    private fun part2Recursive(programStr: String, agg: Long=0): Long? {
        for (a in 0L..7L) {
            val ta = agg or a
            val o = runProgram(programStr, ta, 0, 0)
            if (programStr == o) {
                return ta
            }
            if (programStr.endsWith(o)) {
                val t = part2Recursive(programStr,ta shl 3)
                if (t != null) {
                    return t
                }
            }
