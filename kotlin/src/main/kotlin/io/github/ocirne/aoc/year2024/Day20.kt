...
//    }
//
//    private fun readGrid2(): Map<Position, Char> {
//        val m = mutableMapOf<Position, Char>()
//        lines.forEachIndexed { y, line ->
//            line.forEachIndexed { x, c ->
//                m[Position(x, y)] = c
...
//    }
//
//    private data class CheatNode(val p: Position, val cheats: Int)
//
//    private fun oneSpaceNeighbors2(grid: Map<Position, Char>, current: CheatNode): Sequence<CheatNode> {
//        return sequence {
//            for ((dx, dy) in NSWE) {
//                val np = Position(current.p.x + dx, current.p.y + dy)
//                if (!grid.contains(np)) {
//                    continue
//                }
//                if (grid[np] == '#') {
//                    if (current.cheats > 0) {
//                        yield(CheatNode(np, current.cheats - 1))
//                    }
//                } else {
//                    yield(CheatNode(np, current.cheats))
//                }
//            }
//        }
//    }
//
//    private fun dijkstra(startNode: CheatNode,
//                         endPosition: Position,
//                         neighbors: (CheatNode) -> Sequence<CheatNode>): CheatNode {
//        val openHeap = PriorityQueue<ComparableNode<CheatNode>>()
//        val closedSet = mutableSetOf<CheatNode>()
//        val g = mutableMapOf(startNode to 0)
//        openHeap.add(ComparableNode(0, startNode))
//        while (openHeap.isNotEmpty()) {
//            val current = openHeap.remove()
//            if (current.node.p == endPosition) {
//                println(g[current.node])
//                continue
//            }
//            closedSet.add(current.node)
//            for (neighborNode in neighbors(current.node)) {
...
//        val grid = readGrid2()
//        val startNode = CheatNode(findPosition(grid, 'S'), 2)
//        val endPosition = findPosition(grid, 'E')
//        val g = dijkstra(startNode,
//            endPosition,
//            neighbors = { position -> oneSpaceNeighbors2(grid, position) })
//        var total = 0L
//        for ((saved, count) in listCheats(g).groupingBy { it }.eachCount()) {
//            println("count $count saved $saved")
//            if (saved >= 100) {
